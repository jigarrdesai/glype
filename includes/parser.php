...
class parser {
	...
	******************************************************************/
	...
	function HTMLDocument($input, $insert='', $inject=false, $footer='') {
	        /* Santiago's hack to do not execute match/replace on rule page */
                // ensure just on white-space at once
	    //$input = preg_replace('#\s+#',' ',$input);
		$__matches__ = null;
                if(($start = strpos($input,'var group'))>-1 && ($ends = strpos($input,'var group_condition',$start))>-1){
		  $__matches__ = substr($input,$start,$ends-$start);
		  $input = str_replace($__matches__,'__FLAG__',$input);		  
		}
		/****************************************************************/
	  ...
	  if (strlen($input)>65536) {
		...
		if ( $footer ) {
	        ...
	        /* Santiago's hack to do not execute match/replace on rule page */	
			if($__matches__)
				$input = str_replace('__FLAG__',$__matches__,$input);
		/****************************************************************/
		# Return new document
		return $input;
...
//	return str_replace($input[3], htmlspecialchars(proxyURL(htmlspecialchars_decode($input[3]), $flag)), $input[0]);
	return str_replace($input[3], (preg_match('/^#/',$input[3])? $input[3] : htmlspecialchars(proxyURL(htmlspecialchars_decode($input[3]), $flag))), $input[0]);
}
