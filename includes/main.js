...
window.setInterval(function(){
    ...
    length = imgs.length;
    while(--length>=0)
        if(imgs[length] && !imgs[length].src.match(/browse\.php\?u=/)){
            var src = imgs[length].src,
            root = undefined;
            if((root = window.location.href.match(/browse\.php\?u=([^&]+)/))){
                root = decodeURIComponent(root[1]);
            }else
                root = window.location.href;
            root = root.split('/');
            if(root[root.length-1].match(/\.php/))
                root.splice(root.length-1,1);
            root = root.join('/');
            imgs[length].src = window.parseURL(imgs[length].getAttribute('src').replace(/^\./,root));
        }
},1000/2);
