		...
		if ( isset($parts[1]) ) {
			...
			if ( $headerType == 'set-cookie' && $this->forwardCookies ) {
				...
				$this->setCookie($headerValue);
                        ...
                        if($headerType == 'refresh') {

				$resArr = split(';url=', $headerValue);
				$headerValue = $resArr[0] . ';url=' . proxyURL($resArr[1]);
				$header = $parts[0] . ': ' . $headerValue;
				header('Location: ' . proxyURL($resArr[1]) . ';refresh');die();
			...
			$toForward = array('last-modified','refresh','location',
									 'content-disposition',
									 'content-type',
									 'content-range',
									 'content-language',
			...
			/*if($headerType=='refresh'){
				$resArr = split(';url=', $headerValue);
				//$headerValue = $resArr[0] . ';url=' . proxyURL($resArr[1]);
				$headerValue = proxyURL($resArr[1]);
				$this->headers['location'] = $headerValue;
			}
